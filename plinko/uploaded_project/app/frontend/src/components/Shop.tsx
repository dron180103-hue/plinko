  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-center">
        ...
        <span className="text-[9px] text-amber-400/40 ml-1">🪙</span>
      </div>
                ...
                return (
                  <button key={skin.id}
                    ...
                    {isEquipped ? <div className="text-[7px] text-amber-400 font-bold">✓</div>
                      ...
                      : <div className="text-[7px] text-amber-400 font-bold">{skin.price}🪙</div>}
                  </button>
                );
              })}
            </div>
              ...
              return (
                <button key={theme.id}
                  ...
                  {isEquipped ? <div className="text-[7px] text-amber-400 font-bold">✓ {t.equipped}</div>
                    : isOwned ? <div className="text-[7px] text-emerald-400">{t.owned}</div>
                    : <div className="text-[7px] text-amber-400 font-bold">{theme.price}🪙</div>}
                </button>
              );
            })}
          </div>
              ...
              return (
                <div key={booster.id} className={`flex items-center gap-2 p-2 rounded-xl border ${isActive ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/[0.02] border-white/[0.04]'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {booster.durationMs > 0 && <div className="text-[8px] text-white/15">{Math.floor(booster.durationMs / 60000)} мин</div>}
