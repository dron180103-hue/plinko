...
export interface Ball {
  ...
  bet: number;
  skinId: string;
  stuckTimer: number; // track how long ball hasn't moved
  lastY: number; // last Y position for stuck detection
}
...
const STUCK_THRESHOLD = 60; // frames before considered stuck
const MIN_VELOCITY = 0.3; // minimum velocity to not be considered stuck
...
export function updateBoard(board: PlinkoBoard, deltaTime: number, rtp: number = 75): Ball[] {
  ...
  for (const ball of board.balls) {
    ...
    // Anti-stuck detection: if ball hasn't moved much vertically, increment timer
    const yDelta = Math.abs(ball.y - ball.lastY);
    const speed = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
    if (yDelta < 0.1 && speed < MIN_VELOCITY) {
      ball.stuckTimer += dt;
    } else {
      ball.stuckTimer = Math.max(0, ball.stuckTimer - dt * 2);
    }
    ball.lastY = ball.y;

    // If stuck for too long, give it a nudge downward and sideways
    if (ball.stuckTimer > STUCK_THRESHOLD) {
      ball.vy += 1.5 + Math.random() * 1.5;
      ball.vx += (Math.random() - 0.5) * 3;
      ball.y += 2; // push down past the peg
      ball.stuckTimer = 0;
    }
    ...
    // Peg collision with improved anti-stick
    for (const peg of board.pegs) {
      ...
      if (dist < minDist && dist > 0) {
        ...
        // Push ball out with extra margin to prevent re-collision
        ...
        // Ensure ball always has some downward velocity after collision
        if (ball.vy < 0.3) {
          ball.vy = 0.3 + Math.random() * 0.5;
        }
    ...
    // Force-land balls that have been active too long (safety net)
