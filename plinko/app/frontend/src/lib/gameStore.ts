...
export interface GameState {
  ...
  // Double or Nothing
  doubleOrNothingUsed: number; // per session counter
}
...
export function loadState(): GameState {
  try {
    ...
    if (raw) {
      ...
      if (data.doubleOrNothingUsed === undefined) data.doubleOrNothingUsed = 0;
...
const STREAK_REWARDS: Record<number, number> = { 1: 50, 2: 75, 3: 100, 4: 75, 5: 200, 6: 100, 7: 500 };

export function getStreakReward(day: number): number {
  if (day >= 7) return STREAK_REWARDS[7] || 500;
  return STREAK_REWARDS[day] || 50;
}

export function checkAndUpdateStreak(state: GameState): { reward: number; newStreak: number; isMysteryBox: boolean } {
  const today = new Date().toISOString().slice(0, 10);
  if (state.lastLoginDate === today) {
    return { reward: 0, newStreak: state.loginStreak, isMysteryBox: false };
  }
  ...
  const isMysteryBox = newStreak >= 7 && newStreak % 7 === 0;

  state.loginStreak = newStreak;
  state.lastLoginDate = today;

  return { reward, newStreak, isMysteryBox };
}

export const MAX_DOUBLE_OR_NOTHING = 3;
